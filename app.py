...
# ==============================================================================
st.set_page_config(page_title="KINEMOD-KIT", page_icon="🧪", layout="wide", initial_sidebar_state="expanded")
...
translations = {
    "en": {
        ...
        "welcome_message": "👋 **Welcome!** Please upload your data and select an action in the sidebar to begin.",
        "sidebar_controls": "⚙️ Controls", "sidebar_upload": "Upload data (CSV)", "sidebar_upload_help": "Upload `Model_Data.csv`.",
        ...
        "help_usage_step1_title": "1. Upload Data", "help_usage_step1_text": "Start by uploading your `Model_Data.csv` file in the sidebar. The app will process it automatically.",
    ...
    "id": {
        ...
        "welcome_message": "👋 **Selamat datang!** Silakan unggah data Anda dan pilih tindakan di bilah sisi untuk memulai.",
        "sidebar_controls": "⚙️ Kontrol", "sidebar_upload": "Unggah data (CSV)", "sidebar_upload_help": "Unggah file `Model_Data.csv`.",
        ...
        "help_usage_step1_title": "1. Unggah Data", "help_usage_step1_text": "Mulailah dengan mengunggah file `Model_Data.csv` Anda di bilah sisi. Aplikasi akan memprosesnya secara otomatis.",
...
lang_options = {"English": "en", "Bahasa Indonesia": "id"}; lang_selection = st.sidebar.selectbox("Language / Bahasa", options=list(lang_options.keys()))
st.session_state.lang = lang_options[lang_selection]
def t(key): return translations[st.session_state.lang].get(key, key)

macos_colors = ['#0A84FF', '#30D158', '#FF9F0A', '#FF453A', '#BF5AF2', '#FFD60A', '#64D2FF']
pio.templates["macos"] = go.layout.Template(layout=go.Layout(colorway=macos_colors, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'))
pio.templates.default = "macos"

st.title(f"💻 {t('title')} 📊"); st.subheader(t('subtitle'))
st.caption(t('author_line').format(pd.to_datetime('today').strftime('%Y-%m-%d'))); st.markdown("---")
...
def run_full_analysis(_uploaded_file_content, config, action):
    with open("temp_data.csv", "wb") as f: f.write(_uploaded_file_content)
    reactor = IntegratedReactor("temp_data.csv", config["reactor_constants"])
    reactor.load_and_prepare_data()
    if action == "estimate": reactor.run_parameter_estimation(config["optimization_bounds"])
    elif action == "load":
        if not reactor.load_parameters(): return None, "Could not find `calibrated_params.json`."
...
def run_sensitivity_analysis(analysis_type, model_key, params_uasb, params_filter, params_rbc_orig, params_rbc_ph, _uploaded_file_content, config):
    ...
    reactor = IntegratedReactor("temp_data.csv", config["reactor_constants"])
    reactor.load_and_prepare_data()
    reactor.params_uasb = params_uasb; reactor.params_filter = params_filter; reactor.params_rbc_orig = params_rbc_orig; reactor.params_rbc_ph = params_rbc_ph; reactor.params_estimated = True
    params = {**params_uasb, **params_filter, **(params_rbc_ph if model_key == 'ph' else params_rbc_orig)}
    if not params: return "No parameters available for the selected model.", None
    ...
    if analysis_type == 'GSA':
        ...
        if Mi is None: return failure_message, None
        return None, {'Mi': Mi.to_df(), 'Si': Si, 'problem': problem}
    elif analysis_type == 'Monte Carlo':
        results_df, samples_df = reactor._execute_mc(params, param_keys, model_key)
        if results_df is None: return failure_message, None
...
def optimization_objective_function(inputs, reactor, fixed_params):
    ...
    srr_pred = reactor.uasb_model.predict((cod_in / hrt_uasb_days, fixed_params['vfa_alk']), reactor.params_uasb)
    cod_uasb_pred = cod_in - srr_pred * hrt_uasb_days
    cod_filt_removed = reactor.filter_model.predict((fixed_params['tss_in'], fixed_params['tss_out'], cod_uasb_pred), reactor.params_filter)
    cod_filt_pred = cod_uasb_pred - cod_filt_removed
    final_pred = reactor.rbc_model_ph.predict(
        {'So': cod_filt_pred, 'HRT_days': hrt_rbc_hours / 24.0, 'Xa': fixed_params['xa'], 'Xs': fixed_params['xs'], 'pH': fixed_params['ph']},
...
def plot_interactive_timeseries(df, compliance_limit=None, log_y=False):
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.1, subplot_titles=("UASB", "Filter", "RBC"))
    marker_style = dict(symbol='x', size=8)
    fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_UASB_Eff'], mode='markers', name='UASB Measured', marker=marker_style), row=1, col=1)
    if 'COD_UASB_Pred' in df.columns: fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_UASB_Pred'], mode='lines', name='UASB Predicted'), row=1, col=1)
    fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_Filt_Eff'], mode='markers', name='Filter Measured', marker=marker_style), row=2, col=1)
    if 'COD_Filt_Pred' in df.columns: fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_Filt_Pred'], mode='lines', name='Filter Predicted'), row=2, col=1)
    fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_Final'], mode='markers', name='RBC Measured', marker=marker_style), row=3, col=1)
    if 'COD_Final_Pred_Orig' in df.columns: fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_Final_Pred_Orig'], mode='lines', name='RBC Original Pred.'), row=3, col=1)
    if 'COD_Final_Pred_pH' in df.columns: fig.add_trace(go.Scatter(x=df['Day'], y=df['COD_Final_Pred_pH'], mode='lines', name='RBC pH Pred.'), row=3, col=1)
    if compliance_limit: fig.add_hline(y=compliance_limit, line_dash="dot", line_color="#FF453A", annotation_text="Compliance Limit", annotation_position="bottom right", row=3, col=1)
    fig.update_layout(height=600, title_text=f"<b>{t('timeseries_header')}</b>", hovermode="x unified", legend=dict(yanchor="top", y=0.98, xanchor="left", x=15))
    if log_y: fig.update_yaxes(type="log")
    fig.update_yaxes(title_text="COD (mg/L)", row=1, col=1); fig.update_yaxes(title_text="COD (mg/L)", row=2, col=1); fig.update_yaxes(title_text="COD (mg/L)", row=3, col=1)
    fig.update_xaxes(title_text="Time (Days)", row=3, col=1); return fig
...
def plot_interactive_parity(df, stage_name, actual_col, pred_cols_dict):
    ...
    if not all(col in df.columns for col in required_cols): return go.Figure().update_layout(title=f'Parity Plot: {stage_name} (Missing Data)')
    data = df.dropna(subset=required_cols).copy()
    if data.empty: return go.Figure().update_layout(title=f'Parity Plot: {stage_name} (No valid data)')
    fig = go.Figure()
    all_values = pd.concat([data[actual_col]] + [data[col] for col in pred_cols_dict.values()])
    min_val, max_val = all_values.min(), all_values.max(); padding = (max_val - min_val) * 0.05
    fig.add_shape(type="line", x0=min_val, y0=min_val, x1=max_val, y1=max_val, line=dict(color="#FF453A", dash="dash"), name="1:1 Line")
    for i, (model_name, pred_col) in enumerate(pred_cols_dict.items()):
        fig.add_trace(go.Scatter(x=data[actual_col], y=data[pred_col], mode='markers', name=model_name, marker=dict(color=macos_colors[i]), customdata=data['Day'], hovertemplate='<b>Day %{customdata}</b><br>Measured: %{x:.2f}<br>Predicted: %{y:.2f}<extra></extra>'))
    fig.update_layout(title=f'<b>Parity Plot: {stage_name}</b>', xaxis_title='Measured COD (mg/L)', yaxis_title='Predicted COD (mg/L)', legend_title='Model', xaxis=dict(constrain='domain'), yaxis=dict(scaleanchor="x", scaleratio=1))
    fig.update_xaxes(range=[min_val - padding, max_val + padding]); fig.update_yaxes(range=[min_val - padding, max_val + padding]); return fig
...
def display_final_equations(params_uasb, params_filter, params_rbc_orig, params_rbc_ph):
    ...
    if params_uasb:
        p = params_uasb
        st.markdown(f"""<div class="equation-card"><div class="title">{t('help_uasb_title')}: Substrate Removal Rate</div></div>""", unsafe_allow_html=True)
        st.latex(fr''' SRR = \left( \dfrac{{ \textcolor{{#0A84FF}}{{{p['U_max']:.3f}}} \cdot OLR }}{{ \textcolor{{#0A84FF}}{{{p['K_B']:.3f}}} + OLR }} \right) \cdot \left( \dfrac{1}{{ 1 + \dfrac{{VFA/ALK}}{{ \textcolor{{#0A84FF}}{{{p['K_I']:.3f}}} }} }} \right) ''')
    ...
    if params_filter:
        p = params_filter
        st.markdown(f"""<div class="equation-card"><div class="title">{t('help_filter_title')}: COD Removal</div></div>""", unsafe_allow_html=True)
        st.latex(fr'COD_{{Removed}} = \textcolor{{#0A84FF}}{{{p["R_cod_tss"]:.3f}}} \cdot (TSS_{{in}} - TSS_{{out}}) + \textcolor{{#0A84FF}}{{{p["k_ads"]:.3f}}} \cdot sCOD_{{in}}')
    ...
    st.markdown("---"); st.markdown("#### RBC Models")
    if params_rbc_orig:
        p = params_rbc_orig
        st.markdown(f"""<div class="equation-card"><div class="title">RBC v1.0 (Original)</div></div>""", unsafe_allow_html=True)
        ...
        st.latex(fr'''\dfrac{{S_o - S_e}}{{HRT}} = \dfrac{{\mu_a \cdot X_a \cdot A_b}}{{\textcolor{{#0A84FF}}{{{p['Ya']:.3f}}} \cdot V}} + \dfrac{{\mu_s \cdot X_s}}{{\textcolor{{#0A84FF}}{{{p['Ys']:.3f}}}}}''')
    ...
    if params_rbc_ph:
        ...
        st.markdown(f"""<div class="equation-card"><div class="title">RBC v1.1 (pH-Inhibited)</div></div>""", unsafe_allow_html=True)
        st.latex(fr'''\tau_{{pH}} = \dfrac{{ 1 + 2 \cdot 10^{{{exponent_val:.2f}}} }}{{ 1 + 10^{{pH - \textcolor{{#0A84FF}}{{{p['pH_max']:.2f}}}}} + 10^{{\textcolor{{#0A84FF}}{{{p['pH_min']:.2f}}} - pH}} }}''')
        st.latex(fr'''\mu_a = \tau_{{pH}} \cdot \dfrac{{\textcolor{{#0A84FF}}{{{p['umxa']:.3f}}} \cdot S_e}}{{\textcolor{{#0A84FF}}{{{p['Ku']:.3f}}} + S_e}} \cdot \dfrac{{0.5 \cdot \textcolor{{#0A84FF}}{{{p['Ko']:.3f}}} + \textcolor{{#0A84FF}}{{{p['O']:.3f}}}}}{{ \textcolor{{#0A84FF}}{{{p['Ko']:.3f}}} + \textcolor{{#0A84FF}}{{{p['O']:.3f}}}}}''')
        st.latex(fr'''\dfrac{{S_o - S_e}}{{HRT}} = \dfrac{{\mu_a \cdot X_a \cdot A_b}}{{\textcolor{{#0A84FF}}{{{p['Ya']:.3f}}} \cdot V}} + \dfrac{{\mu_s \cdot X_s}}{{\textcolor{{#0A84FF}}{{{p['Ys']:.3f}}}}}''')
...
# ==============================================================================
if 'reactor' not in st.session_state: st.session_state.reactor = None
if 'sa_results' not in st.session_state: st.session_state.sa_results = None
if 'edited_params' not in st.session_state: st.session_state.edited_params = None
if 'optimizer_results' not in st.session_state: st.session_state.optimizer_results = None

with st.sidebar:
    st.header(t('sidebar_controls')); uploaded_file = st.file_uploader(t('sidebar_upload'), type=["csv"], help=t('sidebar_upload_help'))
    CONFIG = {"reactor_constants": {"V_RBC": 18, "A_RBC": 240}, "optimization_bounds": { "UASB": [(1.4, 75.9), (4.9, 25.0), (1.9, 8.0)], "Filter": [(0.01, 4.0), (0.01, 2.5)], "RBC": [(5, 30), (100, 500), (20, 60), (1, 15), (300, 450), (1, 10), (0.01, 1), (0.01, 2.5)], "RBC_pH": [(5, 50), (50, 100), (20, 30), (1, 5), (1, 5), (1, 10), (0.01, 2.5), (0.01, 2.5), (3.0, 5), (9, 11.5)]}}
    with st.expander(t('sidebar_advanced_config')):
        CONFIG["reactor_constants"]["V_RBC"] = st.number_input(t('sidebar_rbc_vol'), value=18.0)
        CONFIG["reactor_constants"]["A_RBC"] = st.number_input(t('sidebar_rbc_area'), value=240.0)
    col1, col2 = st.columns(2)
    with col1: run_estimation_button = st.button(t('sidebar_calibrate'), type="primary", use_container_width=True, help=t('sidebar_calibrate_help'))
    with col2: load_params_button = st.button(t('sidebar_load'), use_container_width=True, help=t('sidebar_load_help'))

    if st.session_state.reactor:
        st.markdown("---"); st.header(t('what_if_header')); st.info(t('what_if_help'))
        what_if_cod = st.slider(t('what_if_cod'), 5000, 40000, int(st.session_state.reactor.df['COD_in'].mean()), 500)
        what_if_hrt_uasb = st.slider(t('what_if_hrt_uasb'), 1.0, 20.0, st.session_state.reactor.df['HRT_UASB'].mean(), 0.1)
        what_if_hrt_rbc = st.slider(t('what_if_hrt_rbc'), 1.0, 120.0, st.session_state.reactor.df['HRT_hours'].mean(), 0.5)
        srr_pred = st.session_state.reactor.uasb_model.predict((what_if_cod / what_if_hrt_uasb, 0.3), st.session_state.reactor.params_uasb)
        cod_uasb_pred = what_if_cod - srr_pred * what_if_hrt_uasb
        final_pred = st.session_state.reactor.rbc_model_ph.predict({'So': cod_uasb_pred * 0.8, 'HRT_days': what_if_hrt_rbc / 24.0, 'Xa': 400, 'Xs': 200, 'pH': 7.5}, st.session_state.reactor.params_rbc_ph)
        st.metric(label=t('what_if_result'), value=f"{final_pred:.0f} mg/L" if not np.isnan(final_pred) else "N/A")

action_to_perform = None
if run_estimation_button: action_to_perform = "estimate"
if load_params_button: action_to_perform = "load"
if action_to_perform and uploaded_file is not None:
    uploaded_file_content = uploaded_file.getvalue()
    with st.spinner('🔬 Running analysis...'):
        reactor_obj, error = run_full_analysis(uploaded_file_content, CONFIG, action_to_perform)
        if error:
            st.error(error); st.session_state.reactor = None
        else:
            st.session_state.reactor = reactor_obj; st.session_state.sa_results = None; st.session_state.edited_params = None; st.session_state.optimizer_results = None
elif action_to_perform and uploaded_file is None:
    st.warning("Please upload a data file first.", icon="⚠️")
...
if st.session_state.reactor:
    ...
    with tabs[0]:  # Dashboard
        ...
        min_day, max_day = int(df_results['Day'].min()), int(df_results['Day'].max())
        selected_days = st.slider(t('date_slider'), min_day, max_day, (min_day, max_day), key='date_slider_dashboard')
        filtered_df = df_results[(df_results['Day'] >= selected_days[0]) & (df_results['Day'] <= selected_days[1])]
        stages = {"UASB": ("COD_UASB_Eff", "COD_UASB_Pred"), "Filter": ("COD_Filt_Eff", "COD_Filt_Pred"), "RBC (Orig)": ("COD_Final", "COD_Final_Pred_Orig"), "RBC (pH Mod)": ("COD_Final", "COD_Final_Pred_pH")}
        stage_icons = { "UASB": "🦠", "Filter": "✨", "RBC (Orig)": "🔄", "RBC (pH Mod)": "🌿" }
        ...
        for i, (name, (actual_col, pred_col)) in enumerate(stages.items()):
            if pred_col in filtered_df.columns:
                ...
                if not valid_df.empty:
                    r2, rmse, p_value = r2_score(valid_df[actual_col], valid_df[pred_col]), np.sqrt(mean_squared_error(valid_df[actual_col], valid_df[pred_col])), stats.f_oneway(valid_df[actual_col], valid_df[pred_col])[1]
                    with kpi_cols[i]: create_kpi_card( icon=stage_icons.get(name, "📊"), title=name, r2=r2, rmse=rmse, p_value=p_value )
        st.markdown("---"); st.subheader(t('timeseries_header'))
        col_left, col_right = st.columns([1, 2.5])
        with col_left:
            compliance_limit = st.number_input(t('compliance_input'), value=350, min_value=0, step=25)
            compliance_pct = ((filtered_df['COD_Final_Pred_pH'] <= compliance_limit).mean() * 100 if 'COD_Final_Pred_pH' in filtered_df.columns and not filtered_df['COD_Final_Pred_pH'].isna().all() else 0)
            st.metric(t('kpi_compliance_header'), f"{compliance_pct:.1f}%")
        with col_right:
            log_y = st.checkbox(t('log_axis_toggle'))
            st.plotly_chart(plot_interactive_timeseries(filtered_df, compliance_limit, log_y), use_container_width=True)
        st.markdown("---"); st.subheader(t('parity_header'))
        stage_choice = st.selectbox("Select Stage", ["UASB", "Filter", "RBC (Orig)", "RBC (pH Mod)"])
        pred_dict, actual = ({"UASB": "COD_UASB_Pred"}, "COD_UASB_Eff") if stage_choice == "UASB" else ({"Filter": "COD_Filt_Pred"}, "COD_Filt_Eff") if stage_choice == "Filter" else ({"Original": "COD_Final_Pred_Orig"}, "COD_Final") if stage_choice == "RBC (Orig)" else ({"pH Model": "COD_Final_Pred_pH"}, "COD_Final")
        par_col, err_col = st.columns([2, 1])
        with par_col: st.plotly_chart(plot_interactive_parity(filtered_df, stage_choice, actual, pred_dict), use_container_width=True)
        with err_col:
            error_dfs = [pd.DataFrame({ "Error": filtered_df[pred_col] - filtered_df[actual], "Model": model_name }).dropna() for model_name, pred_col in pred_dict.items() if pred_col in filtered_df.columns]
            if error_dfs:
                full_error_df = pd.concat(error_dfs)
                fig_hist = px.histogram(full_error_df, x="Error", color="Model", marginal="box", barmode="overlay", title="Error Distribution")
                fig_hist.add_vline(x=0, line_dash="dash", line_color="red"); st.plotly_chart(fig_hist, use_container_width=True)
    ...
    with tabs[1]: # Model Details
        ...
        param_tab, eq_tab = st.tabs(["Calibrated Parameters", "Governing Equations"])
        with param_tab:
            st.info(t('editable_param_info'))
            param_sets = [("UASB", reactor.params_uasb), ("Filter", reactor.params_filter), ("RBC (Orig)", reactor.params_rbc_orig), ("RBC (pH)", reactor.params_rbc_ph)]
            if 'edited_params' not in st.session_state or st.session_state.edited_params is None:
                all_params = [{"Model": model, "Parameter": key, "Value": value} for model, params in param_sets if params for key, value in params.items()]
                if all_params: st.session_state.edited_params = pd.DataFrame(all_params)
            if st.session_state.edited_params is not None:
                with st.expander("UASB Parameters", expanded=True): uasb_params = st.data_editor(st.session_state.edited_params[st.session_state.edited_params['Model'] == 'UASB'], hide_index=True)
                with st.expander("Filter Parameters"): filter_params = st.data_editor(st.session_state.edited_params[st.session_state.edited_params['Model'] == 'Filter'], hide_index=True)
                with st.expander("RBC Parameters"):
                    rbc_orig_params = st.data_editor(st.session_state.edited_params[st.session_state.edited_params['Model'] == 'RBC (Orig)'], hide_index=True)
                    rbc_ph_params = st.data_editor(st.session_state.edited_params[st.session_state.edited_params['Model'] == 'RBC (pH)'], hide_index=True)
                if st.button(t('rerun_button'), type="primary"):
                    edited_params_df = pd.concat([uasb_params, filter_params, rbc_orig_params, rbc_ph_params])
                    st.session_state.edited_params = edited_params_df
                    with st.spinner("Re-running predictions..."):
                        p_uasb, p_filter, p_rbc_o, p_rbc_ph = [dict(edited_params_df[edited_params_df['Model']==m].set_index('Parameter')['Value']) for m in ["UASB", "Filter", "RBC (Orig)", "RBC (pH)"]]
                        original_df = run_full_analysis.get_cache_value(uploaded_file.getvalue(), CONFIG, action_to_perform)[0].df
                        new_df = reactor.run_predictions_with_new_params(original_df, p_uasb, p_filter, p_rbc_o, p_rbc_ph)
                        st.session_state.reactor.df = new_df; st.success("Predictions updated!")
        with eq_tab: display_final_equations(reactor.params_uasb, reactor.params_filter, reactor.params_rbc_orig, reactor.params_rbc_ph)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tabs[2]: # Methane & Energy
        ...
        # Use a filtered dataframe from the main dashboard for consistency
        filtered_df = df_results[(df_results['Day'] >= st.session_state.get('date_slider_dashboard', (df_results['Day'].min(), df_results['Day'].max()))[0]) & (df_results['Day'] <= st.session_state.get('date_slider_dashboard', (df_results['Day'].min(), df_results['Day'].max()))[1])]
        uasb_cod_removed = (filtered_df['COD_in'] - filtered_df['COD_UASB_Pred']).mean() if 'COD_UASB_Pred' in filtered_df else 0
        uasb_volume = st.number_input(t('uasb_volume_label'), value=25.0, min_value=0.1)
        avg_flow_rate = uasb_volume / (filtered_df['HRT_UASB'].mean() if not filtered_df.empty and 'HRT_UASB' in filtered_df and filtered_df['HRT_UASB'].mean() > 0 else 1)
        ...
        m_cols = st.columns(3)
        m_cols[0].metric(f"{t('cod_removed_label')} (kg/day)", f"{cod_mass_removed_kg_day:.1f}")
        m_cols[1].metric(f"{t('methane_prod_label')} (m³/day)", f"{methane_prod:.1f}")
        m_cols[2].metric(f"{t('energy_potential_label')} (kWh/day)", f"{energy_potential:.1f}")
        st.markdown("</div>", unsafe_allow_html=True)

    with tabs[3]: # Sensitivity
        anim_class = get_animated_class()
        st.markdown(f"<div class='{anim_class}'>", unsafe_allow_html=True)
        st.header(t('sa_header')); st.info(t('sa_info'))
        sa_col1, sa_col2 = st.columns([1, 2])
        with sa_col1:
            sa_type, sa_model = st.selectbox(t('sa_type_select'), ["GSA", "Monte Carlo"]), st.selectbox(t('sa_model_select'), ["RBC Original", "RBC pH-Modified"])
            if st.button(t('run_sa_button'), type="primary", use_container_width=True):
                if uploaded_file:
                    ...
                    with st.spinner(f"Running {sa_type}..."):
                        sa_error, sa_data = run_sensitivity_analysis(sa_type, model_key, reactor.params_uasb, reactor.params_filter, reactor.params_rbc_orig, reactor.params_rbc_ph, uploaded_file.getvalue(), CONFIG)
                        if sa_error: st.error(sa_error); st.session_state.sa_results = None
                        else: st.success("Complete!"); st.session_state.sa_results = sa_data
                else: st.warning("Please upload data first.", icon="⚠️")
        if st.session_state.sa_results:
            with sa_col2:
                sa_data = st.session_state.sa_results
                if 'Mi' in sa_data:
                    st.subheader("Key Findings"); key_cols = st.columns(2)
                    sobol_df = pd.DataFrame({k: sa_data['Si'][k] for k in ['S1', 'S1_conf', 'ST', 'ST_conf']}, index=sa_data['problem']['names'])
                    top2_sobol = sobol_df.sort_values('ST', ascending=False).head(2)
                    if not top2_sobol.empty:
                        with key_cols[0]: st.metric(label=f"1. Most Influential (Sobol ST)", value=top2_sobol.index[0], delta=f"{top2_sobol['ST'].iloc[0]:.3f}")
                        if len(top2_sobol) > 1:
                            with key_cols[1]:
                                st.metric(label=f"2. Most Influential (Sobol ST)", value=top2_sobol.index[1], delta=f"{top2_sobol['ST'].iloc[1]:.3f}")
                    st.plotly_chart(plot_gsa_results(sa_data['Mi'], sa_data['Si'], sa_data['problem']), use_container_width=True)
                elif 'mc_results' in sa_data:
                    st.subheader("Key Findings"); key_cols = st.columns(2)
                    mc_df = sa_data['mc_results']
                    top2_mc = mc_df.reindex(mc_df['Spearman_Correlation'].abs().sort_values(ascending=False).index).head(2)
                    if not top2_mc.empty:
                        with key_cols[0]: st.metric(label=f"1. Most Correlated", value=top2_mc['Parameter'].iloc[0], delta=f"{top2_mc['Spearman_Correlation'].iloc[0]:.3f}")
                        if len(top2_mc) > 1:
                            with key_cols[1]:
                                st.metric(label=f"2. Most Correlated", value=top2_mc['Parameter'].iloc[1], delta=f"{top2_mc['Spearman_Correlation'].iloc[1]:.3f}")
                    st.plotly_chart(plot_mc_results(mc_df), use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with tabs[4]: # Optimizer
        anim_class = get_animated_class()
        st.markdown(f"<div class='{anim_class}'>", unsafe_allow_html=True)
        st.header(t('optimizer_header')); st.info(t('optimizer_intro'))
        ...
        with opt_col1:
            ...
            hrt_uasb_bounds = st.slider(t('what_if_hrt_uasb'), 1.0, 20.0, (1.5, 5.0))
            hrt_rbc_bounds = st.slider(t('what_if_hrt_rbc'), 1.0, 120.0, (6.0, 24.0))
            if st.button(t('optimizer_button'), type='primary', use_container_width=True):
                with st.spinner("Finding optimal settings... this may take some time."):
                    ...
                    initial_guess, fixed_params = [np.mean(b) for b in bounds], {'vfa_alk': df_results['VFA_ALK_Ratio'].mean(), 'tss_in': df_results['TSS_UASB_Eff'].mean(), 'tss_out': df_results['TSS_Filt_Eff'].mean(), 'xa': df_results['Xa'].mean(), 'xs': df_results['Xs'].mean(), 'ph': df_results['pH_Eff_RBC'].mean()}
                    result = minimize(optimization_objective_function, initial_guess, args=(reactor, fixed_params), bounds=bounds, method='L-BFGS-B')
        ...
        with opt_col2:
            if st.session_state.optimizer_results:
                ...
                if res.success:
                    ...
                    r_cols = st.columns(3)
                    r_cols[0].metric("Optimal Influent COD", f"{res.x[0]:.0f} mg/L"); r_cols[1].metric("Optimal UASB HRT", f"{res.x[1]:.2f} days"); r_cols[2].metric("Optimal RBC HRT", f"{res.x[2]:.2f} hours")
                    st.metric("Predicted Minimum Final Effluent", f"{res.fun:.1f} mg/L", help="This is the lowest possible effluent COD the optimizer could find within your constraints.")
                else: st.error(t('optimizer_fail'))
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tabs[5]:  # Help
        ...
        query = st.text_input("🔍 Search Technical Topics", placeholder="e.g., 'UASB model', 'kinetic parameters', 'compliance'")

        def match(q, *texts): return (not q) or any(q.lower() in (t or "").lower() for t in texts)

        # We use a sub-wrapper for search results to re-trigger animation on filter
        search_anim_class = get_animated_class()
        st.markdown(f"<div class='{search_anim_class}'>", unsafe_allow_html=True)
        ...
        if match(query, "uasb", "filter", "rbc", "reactor", "stage", "model"):
            st.subheader("🔬 Reactor Stages & Kinetic Models"); c1, c2, c3 = st.columns(3)
            with c1: st.markdown("""<div class="mini-card"><b>🟤 UASB Reactor</b><br>• <b>Process</b>: Mesophilic (35–38°C) anaerobic digestion for biogas production.<br>• <b>Function</b>: Primary organic load reduction via methanogenesis.<br>• <b>Kinetic Model</b>: A modified Stover–Kincannon substrate utilization model, incorporating a non-competitive inhibition term for the VFA/Alkalinity ratio.</div>""", unsafe_allow_html=True)
            with c2: st.markdown("""<div class="mini-card"><b>🪶 EFB Adsorption Filter</b><br>• <b>Process</b>: Physico-chemical adsorption and filtration.<br>• <b>Function</b>: Removes recalcitrant organics, lipids, and total suspended solids (TSS).<br>• <b>Mechanism</b>: Utilizes lignocellulosic EFB fibers for physical interception and surface chemistry-driven adsorption of pollutants.</div>""", unsafe_allow_html=True)
            with c3: st.markdown("""<div class="mini-card"><b>🧬 RBC Unit</b><br>• <b>Process</b>: Aerobic fixed-film biological oxidation.<br>• <b>Function</b>: Final polishing of effluent to reduce soluble COD to regulatory levels.<br>• <b>Mechanism</b>: Rotating discs support a high-density, attached-growth biofilm, enhancing oxygen transfer and treatment efficiency.</div>""", unsafe_allow_html=True)
        ...
        st.markdown("</div>", unsafe_allow_html=True) # End of search animation wrapper
        ...
        with st.form("feedback_form"):
            ...
            if sent:
                if not msg.strip(): st.warning("⚠️ Please enter a message before submitting.")
                else: st.success("✅ Feedback submitted successfully. Thank you for helping to improve KINEMOD-KIT!")
    ...
    st.image("Diagram.png", caption="Diagram of the multi-stage treatment process.", width=500)